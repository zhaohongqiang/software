...
<!--(end)-->
  ...
  auto _iter = objects.find("pipe_to_navigation");
  if (_iter == objects.end()) throw s::runtime_error("Pipe to navigation not found in locale configuration file!");
  conf::object o = _iter->second;
  navigate_pipe_results shm_navigate_pipe_results;
  shm_navigate_pipe_results.seen = false;
  shm_navigate_pipe_results.north   = o.initial_position[0];
  shm_navigate_pipe_results.east    = o.initial_position[1];
  shm_navigate_pipe_results.depth   = o.initial_position[2];
  shm_navigate_pipe_results.heading = o.initial_position[3]; 
  shm_setg(navigate_pipe_results, shm_navigate_pipe_results);
  AUVLOG("Set initial navigation guess from locale position")
    ...
    if (shm_aslam_settings.reset) {
      ...
      s::for_each(
        ...
        [&](auto& p) {
          ...
          for (unsigned int ind = 2; ind < 6; ind++) p.first.pose[ind] = state[ind]; // TODO FIXME
      }
      );
    };
