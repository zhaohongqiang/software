...
auto get_vision_group_line_data(cuauv::dshm::Group *shm_group,
  ...
  auto up_slope = std::tan(cam.cam->fovy / 2);
  auto up_amt =   -2 * diff_y * up_slope / cam.cam->height;
  auto third_amt = 2 * diff_x * up_slope / cam.cam->height;
...
// TODO Fuse together line_updates and point_managers.
// Should derive from the same subclass.
std::vector<std::function<void()>> line_updates;
...
void draw_loop(GLFWwindow *window) {
  while (!glfwWindowShouldClose(window) && !done) {
    ...
    for (auto &point_manager : point_managers) {
      point_manager->maybe_update();
    }

    for (const auto &line_update : line_updates) {
      line_update();
    }
  ...
  for (const auto &object : objects) {
    ...
    if (!object.exists("model")) {
      if (object.exists("points")) {
        ...
        point_managers.emplace_back(std::make_unique<PointManager>(
        ...
        if (shm_group) {
          ...
          line_updates.push_back(line_update_f);
        }
