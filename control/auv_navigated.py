...
def main(freq=30):
    ...
    vel_x_out = DynamicPID(settings.x_pid_p, settings.x_pid_i,
                           settings.x_pid_d)
    vel_y_out = DynamicPID(settings.x_pid_p, settings.y_pid_i,
                           settings.y_pid_d)
    ...
    while True:
        if (time.time() - start > rate):
            ...
            if settings.position_controls:
                ...
                initial_speed = (x_out**2 + y_out**2)**0.5
