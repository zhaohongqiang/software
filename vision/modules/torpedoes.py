...
options = [options.IntOption('board_blocksize', 901, 0, 2000, lambda x: x % 2 == 1),
           ...
           options.IntOption('board_er_iter', 1, 1, 10),
           options.DoubleOption('board_min_area', 0.01, 0, 1.0),
           options.DoubleOption('board_min_poly_score', 0.5, 0, 1.0),
...
class Torpedoes(ModuleBase):
    ...
    def find_yellow_contours(self, split_image):
        ...
        eroded = cv2.erode(finalThreshed,kernel,iterations = self.options["board_er_iter"])
        finalThreshed = cv2.dilate(eroded,kernel,iterations = self.options["board_er_iter"])
    ...
    def poly_fit(self, c):
        ...
        epsilon = max([min_area_rect[1][0], min_area_rect[1][1]]) * 0.01
        poly = cv2.approxPolyDP(cv2.convexHull(c), 46, True) 
        return poly
    ...
    def num_edge_points(self, poly):
        ...
        for arr in poly:
            ...
            if (point[0] <= 5 or point[0] >= self.img_width - 6 or point[1] <= 5 
                ...
                num_edge_points += 1
                cv2.circle(self.final_contour, point, 10, (0, 0, 255), thickness=10)
                continue
            ...
            for idx, line in enumerate(lines):
                ...
                if inside:
                    num_edge_points += 1
                    cv2.circle(self.final_contour, point, 10, (0,0,0), thickness=10)
                    on_edge = True
                    break

            if not on_edge:
                cv2.circle(self.final_contour, point, 10, (0, 255, 0), thickness=10)
    ...
    def find_board(self, yellow_contours):
        poly_drawing = np.copy(self.mat)
        def valid_board_candidate(c):
            poly = self.poly_fit(c)
            poly = cv2.approxPolyDP(cv2.convexHull(c), 30, True) 
            poly_area = cv2.contourArea(poly)
            cv2.drawContours(poly_drawing, [poly], -1, (255,0,0), 2)
            if poly_area == 0.0:
                return False
            ...
            return (poly.shape[0] >= 4 and poly.shape[0] <= 4 + cutoff_edges and 
                   ...
                   rel_area > self.options["board_min_area"])

        self.post_if_enabled('poly', poly_drawing)
        ...
        board_contour_index, board_contour = max(enumerate(yellow_contours), 
            key=lambda x: cv2.contourArea(x[1]) + self.contour_center(x[1])[1])
        ...
        cv2.circle(self.final_contour, board_center, 2, (255, 255, 255), thickness=2)
        ...
        if board_poly.shape[0] == 4:
            ...
            unskewed = cv2.warpPerspective(self.mat, unskew_mat, (768,768), flags=cv2.INTER_LINEAR) 
        else:
        ...
        text_origin = (10, 30)
        cv2.putText(img, text, text_origin, cv2.FONT_HERSHEY_SIMPLEX, 1.0, (0,0,0), thickness=6)
