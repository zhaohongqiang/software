...
if [ "$SUBMARINE" = "thor" ]; then
  SERVICES=(aslamd seriald gx4d dvld kalmand navigated controld3 shmserver ueye deadman logging uptime sonard visiongui cameras webgui hydromathd modules)
elif [ "$SUBMARINE" = "loki" ]; then
  SERVICES=(aslamd seriald gx1d kalmand navigated controld3 shmserver deadman logging uptime visiongui cameras webgui modules linearizerd)
else
